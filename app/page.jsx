...
const Btn = ({ children, onClick, variant = "primary", style = {}, size = "md" }) => {
  ...
  return (
    <button onClick={onClick} style={{ background: bg, color: variant === "outline" ? colors.primary : "#fff", border: variant === "outline" ? `2px solid ${colors.primary}` : "none", padding: pad, borderRadius: 10, cursor: "pointer", fontWeight: 600, fontSize: size === "sm" ? 13 : size === "lg" ? 17 : 15, ...style }}>
      {children}
    </button>
  );
};
  ...
  <nav style={{ background: colors.primary, color: "#fff", position: "sticky", top: 0, zIndex: 100, boxShadow: "0 2px 12px rgba(30,58,138,0.3)" }}>
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }} onClick={() => setPage("home")}>
        ...
        <span style={{ fontWeight: 800, fontSize: 20 }}>Campus<span style={{ color: colors.accent }}>Ride</span></span>
      </div>
      <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
        {["home","about","routes","pricing"].map(p => (
          <button key={p} onClick={() => setPage(p)} style={{ background: page === p ? "rgba(255,255,255,0.15)" : "transparent", color: "#fff", border: "none", padding: "8px 14px", borderRadius: 8, cursor: "pointer", fontWeight: 500, fontSize: 14, textTransform: "capitalize" }}>{p}</button>
        ))}
        {user ? (
          <button onClick={() => setPage(user.role === "admin" ? "admin" : "dashboard")} style={{ background: colors.secondary, color: "#fff", border: "none", padding: "8px 16px", borderRadius: 8, cursor: "pointer", fontWeight: 600, fontSize: 14 }}>Dashboard</button>
        ) : (
          <>
    ...
    <div style={{ maxWidth: 1200, margin: "0 auto" }}>
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: 40, marginBottom: 40 }}>
        <div>
          ...
          <p style={{ fontSize: 14, lineHeight: 1.7 }}>Smart transportation for university students.</p>
        </div>
        <div>
        ...
        <div>
          ...
          {["Privacy Policy", "Terms of Service"].map(l => <p key={l} style={{ fontSize: 14, marginBottom: 8 }}>{l}</p>)}
        </div>
        <div>
      ...
      <div style={{ borderTop: "1px solid #374151", paddingTop: 24, textAlign: "center" }}>
...
const HomePage = ({ setPage }) => {
  const steps = [
    { n: "01", title: "Register Online", desc: "Create your student account using your university email.", icon: "📝" },
    { n: "02", title: "Get Your Card", desc: "Purchase a $5 transport card at any campus kiosk or online.", icon: "💳" },
    { n: "03", title: "Recharge Anytime", desc: "Top up your balance online — $5, $10, or $20 weekly plans.", icon: "🔄" },
    { n: "04", title: "Ride & Scan", desc: "Tap your card at boarding to deduct the fare automatically.", icon: "🚌" },
  ];
  const features = [
  ...
  return (
    <div>
      <section style={{ background: `linear-gradient(135deg, ${colors.primary} 0%, #1e40af 50%, #1d4ed8 100%)`, color: "#fff", padding: "100px 24px", textAlign: "center" }}>
        <div style={{ maxWidth: 700, margin: "0 auto" }}>
          <div style={{ display: "inline-block", background: "rgba(16,185,129,0.2)", border: "1px solid rgba(16,185,129,0.4)", borderRadius: 30, padding: "6px 18px", marginBottom: 24, fontSize: 13, fontWeight: 600, color: "#6EE7B7" }}>🚌 University Transport System</div>
          <h1 style={{ fontSize: "clamp(36px,5vw,58px)", fontWeight: 900, lineHeight: 1.1, marginBottom: 20 }}>Smart Transport for<br /><span style={{ color: colors.accent }}>University Students</span></h1>
          <p style={{ fontSize: 18, opacity: 0.85, lineHeight: 1.7, marginBottom: 40, maxWidth: 520, margin: "0 auto 40px" }}>Affordable, reliable, and digital-first transportation connecting students to campus every single day.</p>
          <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
            <Btn onClick={() => setPage("register")} variant="accent" size="lg">Get Started Free</Btn>
            <button onClick={() => setPage("routes")} style={{ background: "rgba(255,255,255,0.1)", color: "#fff", border: "2px solid rgba(255,255,255,0.4)", padding: "14px 32px", borderRadius: 10, cursor: "pointer", fontWeight: 600, fontSize: 17 }}>View Routes →</button>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(230px,1fr))", gap: 24 }}>
            {steps.map((s, i) => (
              <Card key={i} style={{ textAlign: "center", position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ width: 56, height: 56, background: colors.primary + "12", borderRadius: 16, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", fontSize: 24 }}>{s.icon}</div>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 60, alignItems: "center" }}>
            <div>
              ...
              <p style={{ color: colors.muted, lineHeight: 1.8, marginBottom: 32 }}>Morning departures starting at 6:30 AM ensure you are always on campus before your first lecture.</p>
      ...
      <section style={{ background: `linear-gradient(135deg,${colors.primary},#1d4ed8)`, color: "#fff", padding: "72px 24px", textAlign: "center" }}>
        ...
        <Btn onClick={() => setPage("register")} variant="accent" size="lg">Register Now</Btn>
      </section>
    </div>
  );
};
    ...
    <div style={{ background: colors.bg }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "72px 24px" }}>
        <Card style={{ marginBottom: 32 }}>
          ...
          <p style={{ color: colors.muted, lineHeight: 1.8, fontSize: 16 }}>Every morning, thousands of university students struggle with expensive rides, unreliable public transport, and long commutes. Many miss lectures, spend excessive amounts on daily transportation, or rely on informal, unsafe carpooling arrangements.</p>
        </Card>
        <Card style={{ marginBottom: 32, borderLeft: `4px solid ${colors.secondary}` }}>
          ...
          <p style={{ color: colors.muted, lineHeight: 1.8, fontSize: 16 }}>CampusRide provides a fully digital, structured transport system exclusively for university students. With prepaid transport cards, fixed routes, scheduled departures, and transparent pricing, students can commute confidently and affordably.</p>
        </Card>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, marginBottom: 48 }}>
          <Card>
            <h3 style={{ color: colors.primary, fontWeight: 700, marginBottom: 10 }}>🎯 Our Mission</h3>
            <p style={{ color: colors.muted, fontSize: 15, lineHeight: 1.7 }}>To make daily campus commuting safe, affordable, and stress-free for every university student.</p>
          </Card>
          <Card>
            <h3 style={{ color: colors.primary, fontWeight: 700, marginBottom: 10 }}>🔭 Our Vision</h3>
            <p style={{ color: colors.muted, fontSize: 15, lineHeight: 1.7 }}>A world where no student misses class because of transport. Smart mobility as a standard feature of every modern university.</p>
          </Card>
        </div>
    ...
    <div style={{ background: colors.bg, minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "60px 24px" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16 }}>
                <div>
                  ...
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    {r.times.map(t => <span key={t} style={{ background: colors.primary + "12", color: colors.primary, padding: "4px 12px", borderRadius: 20, fontSize: 13, fontWeight: 600 }}>{t}</span>)}
                  </div>
                </div>
                <div style={{ textAlign: "right" }}>
        ...
        <Card>
          ...
          <p style={{ color: colors.muted, fontSize: 14, marginBottom: 20 }}>Do not see your neighborhood? Let us know.</p>
          {submitted ? (
            <div style={{ background: colors.secondary + "15", border: `1px solid ${colors.secondary}`, borderRadius: 10, padding: 20, textAlign: "center" }}>
              <p style={{ color: colors.secondary, fontWeight: 700 }}>✅ Suggestion submitted! We will review it within 3–5 business days.</p>
            </div>
          ) : (
...
const PricingPage = ({ setPage }) => {
  const plans = [
    { name: "Starter", price: 5, rides: "~2 rides", popular: false, features: ["2 one-way rides", "Balance never expires", "Transaction history", "Email support"] },
    { name: "Weekly", price: 10, rides: "~4 rides", popular: true, features: ["4 one-way rides", "Balance never expires", "Transaction history", "Priority support", "Route change alerts"] },
    { name: "Pro", price: 20, rides: "~8 rides", popular: false, features: ["8 one-way rides", "Balance never expires", "Full transaction history", "Priority support", "Route alerts", "Bonus 1 free ride"] },
  ];
  return (
    <div style={{ background: colors.bg, minHeight: "100vh" }}>
      <section style={{ background: `linear-gradient(135deg,${colors.primary},#1d4ed8)`, color: "#fff", padding: "60px 24px", textAlign: "center" }}>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "60px 24px" }}>
        <div style={{ background: colors.primary + "10", borderRadius: 14, padding: 20, marginBottom: 48, display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <div>
            ...
            <p style={{ color: colors.muted, fontSize: 14 }}>Every student pays a one-time $5 card fee. The $5 is added to your ride balance.</p>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: 24, marginBottom: 48 }}>
          {plans.map(p => (
            <div key={p.name} style={{ background: p.popular ? colors.primary : "#fff", borderRadius: 18, padding: 28, border: p.popular ? "none" : `1px solid ${colors.border}`, position: "relative", boxShadow: p.popular ? `0 8px 32px ${colors.primary}40` : "0 1px 4px rgba(0,0,0,0.06)" }}>
              ...
              <button onClick={() => setPage("register")} style={{ width: "100%", background: p.popular ? colors.accent : colors.primary, color: "#fff", border: "none", padding: "12px", borderRadius: 10, cursor: "pointer", fontWeight: 700, fontSize: 15 }}>Get Started</button>
            </div>
          ))}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
          <Card>
            ...
            <p style={{ color: colors.muted, fontSize: 14, lineHeight: 1.7 }}>Report lost cards immediately. Balance transferred to new card after $2 replacement fee.</p>
          </Card>
          <Card>
            <h3 style={{ fontWeight: 700, color: colors.dark, marginBottom: 10 }}>↩️ Refund Policy</h3>
            <p style={{ color: colors.muted, fontSize: 14, lineHeight: 1.7 }}>Unused balance refunded upon graduation. Submit request 5 business days before final exam.</p>
          </Card>
        </div>
      </div>
    </div>
    ...
    <div style={{ minHeight: "calc(100vh - 64px)", background: colors.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ width: "100%", maxWidth: 420 }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          <div style={{ width: 56, height: 56, background: colors.primary, borderRadius: 16, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}><BusIcon /></div>
        ...
        <Card>
          {type === "register" && <input placeholder="Full Name" value={form.name} onChange={e => setForm({...form, name: e.target.value})} style={{ width: "100%", padding: "12px 14px", borderRadius: 10, border: `1px solid ${colors.border}`, fontSize: 15, outline: "none", marginBottom: 14, boxSizing: "border-box" }} />}
          ...
          <input placeholder="Password" type="password" value={form.password} onChange={e => setForm({...form, password: e.target.value})} style={{ width: "100%", padding: "12px 14px", borderRadius: 10, border: `1px solid ${colors.border}`, fontSize: 15, outline: "none", marginBottom: 20, boxSizing: "border-box" }} />
          {error && <p style={{ color: colors.danger, fontSize: 13, marginBottom: 12 }}>{error}</p>}
          <Btn onClick={handleSubmit} variant="primary" style={{ width: "100%", padding: "13px" }}>{type === "login" ? "Sign In" : "Create Account"}</Btn>
          <p style={{ textAlign: "center", marginTop: 20, fontSize: 14, color: colors.muted }}>
            {type === "login" ? "No account? " : "Already registered? "}
            <span onClick={() => setPage(type === "login" ? "register" : "login")} style={{ color: colors.primary, cursor: "pointer", fontWeight: 600 }}>{type === "login" ? "Register" : "Sign In"}</span>
          </p>
          {type === "login" && (
            <div style={{ marginTop: 16, padding: 12, background: colors.primary + "08", borderRadius: 8, fontSize: 13, color: colors.muted }}>
              <strong>Demo:</strong> Use <code>admin@uni.edu</code> for admin, any other email for student.
            </div>
          )}
        </Card>
      </div>
...
const Dashboard = ({ user, setUser, setPage }) => {
  ...
  const handleRecharge = () => { setUser({ ...user, balance: user.balance + rechargeAmt }); setRecharging(false); };
  const sidebarItems = [
  ...
  return (
    <div style={{ display: "flex", minHeight: "calc(100vh - 64px)", background: colors.bg }}>
      <div style={{ width: 240, background: colors.primary, color: "#fff", padding: "24px 12px", display: "flex", flexDirection: "column", gap: 4 }}>
        ...
        <div style={{ flex: 1 }} />
        <button onClick={() => { setUser(null); setPage("home"); }} style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 14px", borderRadius: 10, background: "rgba(239,68,68,0.15)", color: "#FCA5A5", border: "none", cursor: "pointer", fontSize: 14 }}>🚪 Logout</button>
      </div>
      <div style={{ flex: 1, padding: 32, overflow: "auto" }}>
        {activeTab === "overview" && (
          <div>
            <h1 style={{ fontSize: 26, fontWeight: 800, color: colors.dark, marginBottom: 4 }}>Good morning, {user.name.split(" ")[0]}! 👋</h1>
            <p style={{ color: colors.muted, marginBottom: 28 }}>Here is your transport overview for today.</p>
              ...
              <Card>
                ...
                {demoTransactions.slice(0, 4).map(t => (
                  <div key={t.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: `1px solid ${colors.border}` }}>
                    ...
                    <span style={{ fontWeight: 700, color: t.amount > 0 ? colors.secondary : colors.danger }}>{t.amount > 0 ? "+" : ""}${Math.abs(t.amount).toFixed(2)}</span>
                  </div>
                ))}
              </Card>
                ...
                <tbody>
                  {demoTransactions.map(t => (
                    <tr key={t.id} style={{ borderBottom: `1px solid ${colors.border}` }}>
                      ...
                      <td style={{ padding: "14px 16px", fontWeight: 700, color: t.amount > 0 ? colors.secondary : colors.danger }}>{t.amount > 0 ? "+" : ""}${Math.abs(t.amount).toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
          <Card style={{ width: 360, boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}>
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
              {[5,10,20].map(amt => (
                <button key={amt} onClick={() => setRechargeAmt(amt)} style={{ flex: 1, padding: 14, borderRadius: 10, border: `2px solid ${rechargeAmt === amt ? colors.primary : colors.border}`, background: rechargeAmt === amt ? colors.primary + "12" : "#fff", cursor: "pointer", fontWeight: 700, color: rechargeAmt === amt ? colors.primary : colors.muted, fontSize: 16 }}>${amt}</button>
              ))}
            </div>
            <p style={{ color: colors.muted, fontSize: 13, marginBottom: 20 }}>Add ${rechargeAmt} to your balance (demo)</p>
...
const AdminPanel = ({ user, setUser, setPage }) => {
  ...
  const saveBalance = (id) => { setStudents(students.map(s => s.id === id ? { ...s, balance: parseFloat(newBalance) || s.balance } : s)); setEditingId(null); };
  const sidebarItems = [
  ...
  return (
    <div style={{ display: "flex", minHeight: "calc(100vh - 64px)", background: colors.bg }}>
      <div style={{ width: 240, background: "#0F172A", color: "#fff", padding: "24px 12px", display: "flex", flexDirection: "column", gap: 4 }}>
        ...
        <div style={{ flex: 1 }} />
        <button onClick={() => { setUser(null); setPage("home"); }} style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 14px", borderRadius: 10, background: "rgba(239,68,68,0.1)", color: "#FCA5A5", border: "none", cursor: "pointer", fontSize: 14 }}>🚪 Logout</button>
      </div>
      <div style={{ flex: 1, padding: 32, overflow: "auto" }}>
        {activeTab === "overview" && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
              <Card>
                ...
                {[["Painesville", 88], ["Broadville", 74], ["Baton Rougeville", 91]].map(([city, pct]) => (
                  <div key={city} style={{ marginBottom: 16 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                      <span style={{ fontSize: 14, fontWeight: 600, color: colors.dark }}>{city}</span>
                      <span style={{ fontSize: 13, color: colors.muted }}>{pct}%</span>
                    </div>
                    <div style={{ background: colors.border, borderRadius: 4, height: 8 }}>
                      <div style={{ background: colors.primary, borderRadius: 4, height: 8, width: `${pct}%` }} />
                    </div>
                  </div>
                ))}
              </Card>
          ...
          <div>
            ...
            <Card>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ borderBottom: `2px solid ${colors.border}` }}>
                    {["Student","Email","Route","Balance","Status","Actions"].map(h => <th key={h} style={{ padding: "12px 14px", textAlign: "left", fontSize: 13, fontWeight: 700, color: colors.muted }}>{h}</th>)}
                  </tr>
                </thead>
                <tbody>
                  {students.map(s => (
                    <tr key={s.id} style={{ borderBottom: `1px solid ${colors.border}` }}>
                      ...
                      <td style={{ padding: "14px" }}><Btn onClick={() => { setEditingId(s.id); setNewBalance(s.balance.toString()); }} variant="outline" size="sm">Edit Balance</Btn></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {activeTab === "routes" && (
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <button style={{ background: "transparent", border: `2px dashed ${colors.border}`, borderRadius: 16, padding: 20, cursor: "pointer", color: colors.muted, fontWeight: 600, fontSize: 15 }}>+ Add New Route</button>
            </div>
          </div>
        )}
        {activeTab === "stops" && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
              <Card>
                ...
                {[{area:"Northside Park",route:"Painesville",by:"student@uni.edu"},{area:"West Village",route:"Broadville",by:"user2@uni.edu"},{area:"Riverside Dr",route:"Baton Rougeville",by:"user3@uni.edu"}].map((s,i) => (
